## Accuracy, recency, and sourcing (REQUIRED)

When a request depends on recency (e.g., "latest", "current", "today", "as of now"):

1. **Establish the current date/time** and state it explicitly in ISO format.
   - Preferred: `date -Is` (timestamp).

2. **Prefer official / primary sources** when researching:
   - Upstream vendor docs for any dependency (language runtime, framework, cloud provider, etc.)

3. **Prefer the most recent authoritative information**:
   - Use the newest versioned docs, release notes, or changelogs.
   - Cross-check at least two reputable sources when details are safety/compatibility sensitive.

### Context7 MCP

- Use Context7 when you need library/API docs.
- If known, pin the library with slash syntax (e.g., `use library /supabase/supabase`).
- Mention the target version.
- Fetch minimal targeted docs; summarize (no large dumps).

### Web search policy

- Enable and use web search only when it materially improves correctness (e.g., up-to-date APIs, recent advisories, release notes).
- Prefer official docs and primary sources; otherwise use Context7 MCP or reputable, widely-cited references.
- Record source dates (publish/release dates) when relevant.

## Default autonomy and safety

- Default to read-only exploration and analysis.
- When edits are needed, prefer **workspace-scoped** write access and keep changes inside the repo.
- When interacting with remote APIs, you must use READ-only calls, unless explicitily instructed otherwise by the user. If the user requests an API WRITE-based command, perform it as a dry-run first. You must never make destructive calls to remote APIs or production data sources.

### Editing files

- Make the smallest safe change that solves the issue.
- Preserve existing style and conventions.
- Prefer patch-style edits (small, reviewable diffs) over full-file rewrites.
- After making changes, run the project’s standard checks when feasible (format/lint, unit tests, build/typecheck).

### Reading project documents (PDFs, uploads, long text, CSVs, etc)

- Read the full document first.
- Draft the output.
- **Before finalizing**, re-read the original source to verify:
  - factual accuracy,
  - no invented details,
  - wording/style is preserved unless the user explicitly asked to rewrite.
- If paraphrasing is required, label it explicitly as a paraphrase.

### Container-first policy (REQUIRED)

- Codex must **never** install system packages on the host unless explicitly instructed.
- Prefer container images to supply all tooling used by the project.
- For code projects and dependencies: **use containers by default**.
- If the repo has an existing container workflow (Dockerfile/compose/Makefile targets), follow it.
- If the repo has no container workflow, create a minimal one.
- Keep repo-specific container details in the repo’s `AGENTS.md`.

### Secrets and sensitive data

- Never print secrets (tokens, private keys, credentials) to terminal output.
- Do not request users paste secrets.
- Avoid commands that might expose secrets (e.g., dumping env vars broadly, `cat ~/.ssh/*`).
- Prefer existing authenticated CLIs; redact sensitive strings in any displayed output.

## Baseline workflow

- Start every task by determining:
  1. Goal + acceptance criteria.
  2. Constraints (time, safety, scope).
  3. What must be inspected (files, commands, tests, docs).
  4. Whether the request depends on **recency** (if yes, apply the "Accuracy, recency, and sourcing" rules).
  5. If requirements are ambiguous, ask targeted clarifying questions before making irreversible changes.

## CONTINUITY.md (REQUIRED)

Maintain a single continuity file for the current workspace: `.agent/CONTINUITY.md`.

- `.agent/CONTINUITY.md` is a living document and canonical briefing designed to survive compaction; do not rely on earlier chat/tool output unless it's reflected there.

- At the start of each assistant turn: read `.agent/CONTINUITY.md` before acting.

### File Format

Update `.agent/CONTINUITY.md` only when there is a meaningful delta in:

  - `[PLANS]`: "Plans Log" is a guide for the next contributor as much as checklists for you.
  - `[DECISIONS]`: "Decisions Log" is used to record all decisions made.
  - `[PROGRESS]`: "Progress Log" is used to record course changes mid-implementation, documenting why and reflecting upon the implications.
  - `[DISCOVERIES]`: "Discoveries Log" is for when when you discover optimizer behavior, performance tradeoffs, unexpected bugs, or inverse/unapply semantics that shaped your approach, capture those observations with short evidence snippets (test output is ideal).
  - `[OUTCOMES]`: "Outcomes Log" is used at completion of a major task or the full plan, summarizing what was achieved, what remains, and lessons learned.

### Anti-drift / anti-bloat rules

- Facts only, no transcripts, no raw logs.
- Every entry must include:
  - a date in ISO timestamp (e.g., `2026-01-13T09:42Z`)
  - a provenance tag: `[USER]`, `[CODE]`, `[TOOL]`, `[ASSUMPTION]`
  - If unknown, write `UNCONFIRMED` (never guess). If something changes, supersede it explicitly (don't silently rewrite history).
- Keep the file bounded, short and high-signal (anti-bloat). 
- If sections begin to become bloated, compress older items into milestone (`[MILESTONE]`) bullets.

## Repository workflows and commands

### Workspace layout

- `frontend/`: React/Vite public site. `/` is the canonical one-page route; `/about` redirects to `/#resume`; `/project/:slug`, `/rnd`, `/rnd/:slug`, and `/contact` remain active routes.
- `ParticleEarth/`: standalone React Three Fiber app with a preserved synced build in `frontend/public/particle-earth`. The dark kinetic homepage uses original Sanity imagery and does not mount the earth iframe.
- `sanity/`: Sanity Studio content backend and schemas.

### Frontend

- From `frontend/`, use `npm run dev` for local work, `npm run preview` to serve a built bundle locally, `npm run lint` and `npm run build` for standard verification, and `npm run generate-sitemap` when sitemap output is needed.
- `npm run generate-sitemap` writes `frontend/public/sitemap.xml` and uses the Sanity env vars plus optional `SITE_URL` / `VITE_SANITY_API_VERSION` overrides when those are relevant.
- `npm run design-audit` can auto-detect a local dev server on `http://localhost:5173` through `:5180`, or use an explicit `BASE_URL`, but browser launch may be blocked in this sandbox; use browser/Playwright verification when feasible.
- The active homepage is `src/pages/Home.jsx` with `src/components/home/KineticLanding.jsx` and `kinetic.css`. It is one continuous page: Particle Sea opening, selected work and attributed SIT awards, inline project index, experiments, About/capabilities and contact. Keep original Sanity titles, imagery and encoded destinations; the old carousel/contour components are inactive.
- After a frontend build, `docker compose exec app node scripts/verifyKinetic.mjs` temporarily serves the production bundle, verifies laptop/phone images and overflow, and records screenshots, lab LCP/CLS and motion frame samples in ignored `frontend/artifacts/kinetic/`. It retains Sanity's authorized localhost:5173 origin inside the browser and intercepts telemetry/contact requests. This is not a field Web Vitals or INP measurement. `verifyOverhaul.mjs` is historical and targets the retired carousel.
- The Particle Sea opening uses `frontend/public/media/particle-sea-loop.mp4`, converted from the user's supplied video, with the Sanity image as fallback; a downloadable GIF sits alongside it. `frontend/scripts/createParticleSeaLoop.sh` reproduces both with FFmpeg in the container. See `docs/design/kinetic/particle-sea-loop.md` for source and conversion details. The production audit also verifies muted inline video playback and the loop boundary.

### ParticleEarth embed

- Prefer the canonical sync command from `frontend/`: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\syncParticleEarth.ps1`. It runs `npm run build` in `../ParticleEarth` and copies the built bundle into `frontend/public/particle-earth`.
- If the earth bundle is already built and only needs recopying, run `node scripts/syncParticleEarth.mjs` from `frontend/`.
- When working inside `ParticleEarth/` directly, use `npm run dev` for the standalone app, `npm run preview` for a built preview, and `npm run test` to run both unit and e2e suites together.
- If the elevation texture needs regeneration, run `python scripts/generate_etopo_heightmap.py` from `ParticleEarth/`; it reads `output/etopo1-bedrock-2048x1024.tif` and rewrites `public/earth-elevation.png`.
- Do not hand-edit `frontend/public/particle-earth`; rebuild and sync from `ParticleEarth/` instead.
- Verify `ParticleEarth/` with `npm run lint`, `npm run test:unit`, `npm run build`, and `npm run test:e2e` when browser support is available.

### Container workflow

- `ParticleEarth/` has a Docker Compose workflow in `ParticleEarth/compose.yaml`: `docker compose up -d` starts the dev server on `http://localhost:5173` and auto-installs dependencies into the named `/app/node_modules` volume when needed; use `docker compose exec app npm run build`, `docker compose exec app npm run lint`, `docker compose exec app npm run test:unit`, and `docker compose exec app npm run test:e2e` for verification.
- If the `ParticleEarth/` container is already running and dependencies change, refresh the named `/app/node_modules` volume with `docker compose exec app npm install`.
- `frontend/compose.yaml` uses the Playwright 1.58.1 Noble image (matching the existing browser-test dependency) for Node and Chromium tooling. From frontend, `docker compose up -d` installs the locked dependencies into a named volume and serves localhost:5173. Run `docker compose exec app npm run lint`, `npm run test:unit`, `npm run test:e2e`, `npm run build`, and `npm run design-audit` through the same exec prefix. Restart the service after lockfile changes to rerun npm ci. Host system installs are unnecessary. Sanity has no container workflow yet; create one if Studio work requires setup.

### Sanity Studio

- From `sanity/`, use `npm run dev` for the local studio, `npm run start` for Sanity's start server, and `npm run build` for verification.
- `npm run deploy` and `npm run deploy-graphql` are remote writes; only run them when explicitly instructed, and dry-run or confirm first when possible.

### Git

- If Git reports dubious ownership in this Windows workspace, use command-scoped `safe.directory`, for example `git -c safe.directory=C:/Users/USER/Documents/Portfolio_website/New_Portfolio status`. Use the nested `ParticleEarth` path when running Git inside that repo.

## Definition of done

A task is done when:

- the requested change is implemented or the question is answered,
  - verification is provided:
  - build attempted (when source code changed),
  - linting run (when source code changed),
  - errors/warnings addressed (or explicitly listed and agreed as out-of-scope),
  - plus tests/typecheck as applicable,
- documentation is updated exhaustively for impacted areas,
- impact is explained (what changed, where, why),
- follow-ups are listed if anything was intentionally left out.
- `.agent/CONTINUITY.md` is updated if the change materially affects goal/state/decisions.
